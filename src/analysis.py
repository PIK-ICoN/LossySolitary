...
from src.sim_functions import find_sim_dir, gen_swing_rhs, def_rc_gen, def_rc_gen_global, def_rc_gen_square, \
    def_swing_ob, plot_fps, detect_clusters
...
def analyze_multistability(swingpar,
    ...
    if "global" in run and swingpar.topology is not "ensemble":
        ...
        rc_gen = def_rc_gen_global(swingpar, fps, alphas, freq_var=f_vars[1], phase_var=np.pi)
    ...
    def ts_witness(brp, batch, run, batch_folder, states, rc):
        if debug or np.random.randint(10) < 1:
            ...
            if n_clusters > 1:
                ...
                #print(("save ts to ", filename), file=sys.stderr)
